use crate::macros::{
    def_vec_struct, impl_vec_add, impl_vec_add_assign, impl_vec_index, impl_vec_neg, impl_vec_new,
};

def_vec_struct!(Vec4<f32, 4> { x, y, z, w });

impl_vec_new!(Vec4<f32> { x, y, z, w });

impl_vec_index!(Vec4<f32, 4> { 0 => x, 1 => y, 2 => z, 3 => w });

impl_vec_neg!(Vec4 { x, y, z, w });

impl_vec_add!(Vec4 { x, y, z, w });

impl_vec_add_assign!(Vec4 { x, y, z, w });

#[cfg(test)]
mod tests {
    use crate::macros::{
        test_vec_add, test_vec_add_assign, test_vec_index, test_vec_index_out_of_bounds,
        test_vec_neg, test_vec_new,
    };
    use crate::Vec4;

    test_vec_new!(Vec4 {
        a { x: 1.0, y: 2.0, z: 3.0, w: 4.0 },
        b { x: 5.0, y: 6.0, z: 7.0, w: 8.0 },
    });

    test_vec_index!(Vec4 {
        a { 0 => x: 9.0 = 0.7, 1 => y: 0.0 = 0.8, 2 => z: 0.1 = 0.9, 3 => w: 0.2 = 1.0 },
        b { 0 => x: 0.3 = 1.1, 1 => y: 0.4 = 1.2, 2 => z: 0.5 = 1.3, 3 => w: 0.6 = 1.4 },
    });

    test_vec_index_out_of_bounds!(Vec4 { 1.5, 1.6, 1.7, 1.8 } 4 = 1.9);

    test_vec_neg!(Vec4 {
        a { -2.0, -2.1, -2.2, -2.3 } => { 2.0, 2.1, 2.2, 2.3 },
        b { 2.4, 2.5, 2.6, 2.7 } => { -2.4, -2.5, -2.6, -2.7 },
    });

    test_vec_add!(Vec4 {
        a { 2.8, 2.9, 3.0, 3.1 } + b { -3.2, 3.3, 3.4, -3.5 } = c,
        d { -3.6, 3.7, 3.8, -3.9 } + e { 4.0, 4.1, 4.2, 4.3 } = f,
    });

    test_vec_add_assign!(Vec4 {
        a { 4.4, -4.5, -4.6, 4.7 } += b { -4.8, -4.9, 5.0, 5.1 },
        c { -5.2, 5.3, 5.4, -5.5 } += d { 5.6, 5.7, -5.8, -5.9 },
    });
}
